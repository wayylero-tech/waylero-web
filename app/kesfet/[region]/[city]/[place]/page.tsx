...
const slugify = (text: string) => {
  ...
  return text.toString().toLowerCase().replace(/[çğışüö]/g, (m) => trMap[m])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${foundPlace.name?.tr} | Waylero`, description: foundPlace.description?.tr?.slice(0, 155) };
}
...
export default async function Page({ params }: Props) {
  ...
  const schema = {
    ...
    address: { "@type": "PostalAddress", addressLocality: cityKey, addressCountry: region === "turkiye" ? "TR" : region.toUpperCase() }
  };
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8 space-y-10">
      <header className="border-b pb-8">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-gray-900">{foundPlace.name?.tr}</h1>
      </header>
      <section>
        ...
        <div className="bg-blue-50 rounded-[2rem] p-8 border border-blue-100 h-fit">
          ...
          <ul className="space-y-3">
            {foundPlace.activities?.tr?.map((act: string, i: number) => (
              <li key={i} className="bg-white p-3 rounded-xl text-blue-800 text-sm font-semibold shadow-sm">• {act}</li>
            ))}
          </ul>
        </div>
      </div>
      {foundPlace.latitude && foundPlace.longitude && (
        <section className="space-y-4">
          ...
          <div className="rounded-[2.5rem] overflow-hidden border-4 border-white shadow-2xl h-[450px]">
            <iframe src={`https://maps.google.com/maps?q=${foundPlace.latitude},${foundPlace.longitude}&z=15&output=embed`} width="100%" height="100%" style={{ border: 0 }} loading="lazy" />
          </div>
        </section>
      )}
