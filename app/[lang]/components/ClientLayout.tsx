...
export default function ClientLayout({
  children,
  lang, // Server side'dan gelen kesin dil
}: {
  ...
  // ✅ ARTIK TEK KAYNAK: Server'dan gelen lang propu
  ...
  const switchLanguage = (langToSet: "tr" | "en") => {
    ...
    const newPath = `/${langToSet}${segments.length > 0 ? "/" + segments.join("/") : ""}`;
    window.location.href = newPath;
  };

  const getLocalizedLink = (path: string) => {
    const cleanPath = path.startsWith("/") ? path : `/${path}`;
    return `/${activeLang}${cleanPath === "/" ? "" : cleanPath}`;
  };
  ...
  useEffect(() => {
    const handleScroll = () => setShouldBeSmall(window.scrollY > 50);
  ...
  return (
    <div className={`${geistSans.variable} ${geistMono.variable} antialiased min-h-screen flex flex-col`}>
      {/* HEADER VE FOOTER TASARIMIN AYNI KALIYOR */}
      <header className={`sticky top-0 z-50 bg-[#F9F7F2]/95 backdrop-blur-md shadow-sm border-b border-black/5 px-4 transition-all duration-300 ${shouldBeSmall ? "py-1.5" : "py-3"}`}>
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            <Link href={getLocalizedLink("/")}>
              <img src="/assets/logo/logo.webp" width={224} height={56} className={`object-contain transition-all duration-300 ${shouldBeSmall ? "h-10" : "h-12"}`} alt="Waylero logo" />
            </Link>
          </div>
          <div className="flex-1 flex justify-end">
            <div className="flex items-center gap-1 border border-black/10 rounded-full p-1 bg-black/5 text-[11px] font-medium">
              <button onClick={() => switchLanguage("tr")} className={`px-2 py-0.5 rounded-full ${activeLang === "tr" ? "bg-white text-black shadow-sm" : "text-gray-500 hover:text-black"}`}>🇹🇷</button>
              <button onClick={() => switchLanguage("en")} className={`px-2 py-0.5 rounded-full ${activeLang === "en" ? "bg-white text-black shadow-sm" : "text-gray-500 hover:text-black"}`}>🇺🇸</button>
            </div>
          </div>
        </div>
      </header>
