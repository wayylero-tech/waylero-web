...
export async function generateMetadata({
  ...
  const slugMap = Object.fromEntries(
    Object.entries(cityMap).map(([k, v]) => [
      slugify(k),
      {
        id: v,
        name: k,
  ...
  const pageUrl =
    `${BASE_SITE_URL}/${currentLang}/aktiviteler/${citySlug}`;
...
export default async function CityActivitiesPage({
  ...
  try {
    ...
    console.error(
      "Şehir sayfası doğrudan veri çekme hatası:",
      err
    );
  ...
  const jsonLd = {
    ...
    name:
      currentLang === "tr"
        ? `${cityName} Konserleri ve Etkinlikleri`
        : `${cityName} Concerts & Events`,

    description:
      currentLang === "tr"
        ? `${cityName} konserlerini, tiyatro gösterilerini ve kültür sanat etkinliklerini keşfedin. Yaklaşan etkinlikleri inceleyin.`
        : `Discover concerts, theater shows and cultural events in ${cityName}. Explore upcoming events and activities.`,
...

        {
          ...
          name:
            currentLang === "tr"
              ...
              : "Concerts & Events",
          item:
            `${BASE_SITE_URL}/${currentLang}/aktiviteler`,
        },
        ...
        <div className="w-full">
          {cityEvents.length === 0 && (
            <div className="bg-amber-50/60 border-b p-8 text-center flex flex-col items-center justify-center min-h-[50vh]">
              <h1 className="font-bold text-2xl text-amber-900 mb-2">
                {cityName}
              </h1>
