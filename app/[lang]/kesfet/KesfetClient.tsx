...
import countrySummaries from "@/data/country-summaries.json";
...
export default function KesfetClient({ lang }: { lang: string }) {
  ...
  const summaries = countrySummaries as CountrySummary[];

  const getCountryName = (slug: string) => {
    return (
      countryNames[slug]?.[isEn ? "en" : "tr"] ||
      slug.replace(/-/g, " ")
    );
  };

  const getCountrySummary = (slug: string) => {
    return summaries.find((country) => country.id === slug);
  };
      ...
      <section className="container mx-auto px-6 -mt-24 pb-32 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12">
          ...
          {allCountries.map(([slug, data]: any, index) => {
            ...
            const countryName =
              summary?.country_name?.[isEn ? "en" : "tr"] ||
              getCountryName(slug);
            ...
            return (
              <Link
                ...
                className="group flex flex-col bg-white rounded-[3rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3"
              >
