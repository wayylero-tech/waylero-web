...
const getCloudinaryUrl = (imagePath: string, width: number) => {
  if (!imagePath) return "";
...
const formatCityName = (name: string, lang: string) => {
  ...
  if (lang === "tr") {
    return cityNameMapTR[normalized] || normalized.replace(/\b\w/g, (letter) => letter.toUpperCase());
  }
  return normalized
...
export default function RegionClient({
    ...
    return Object.entries(data)
      .map(([cityKey, places]) => {
        ...
        return {
          ...
          coverImage,
          places: places.filter(Boolean).slice(0, 3),
        };
      })
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollBottom) {
        setVisibleCount((prev) => Math.min(prev + 6, countryCities.length));
      }
    };
  ...
  const getLocalizedLink = (urlPath: string) => (isEn ? `/en${urlPath}` : `/tr${urlPath}`);
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-24 pb-48 bg-[linear-gradient(110deg,#fdfaf7_50%,#e6f4f9_50%)]">
        <div className="container mx-auto px-6 text-center">
          ...
          <p className="text-xl text-gray-500 max-w-2xl mx-auto font-medium leading-relaxed italic">
            {isEn ? `Explore ${regionName}. Discover the most popular places.` : `${regionName}’yi keşfedin. En popüler yerleri görün.`}
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-6 -mt-24 pb-32 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {countryCities.slice(0, visibleCount).map((city, index) => (
            <Link
              key={city.slug}
            ...
            >
              ...
              <div className="relative aspect-[4/5] overflow-hidden bg-gray-900">
                {city.coverImage ? (
                  <img
                    src={getCloudinaryUrl(city.coverImage, 400)}
                    alt={city.name}
              ...
              <div className="bg-white px-8 md:px-10 pt-8 pb-10">
                <div className="flex items-center gap-2 mb-7">
                  ...
                  <h3 className="text-lg md:text-xl font-bold text-gray-900">
                    {city.name} {t.placesTitle}
                  </h3>
                </div>

                <div className="space-y-6">
                  {city.places.map((place, placeIndex) => {
                    const placeName = place.name?.[isEn ? "en" : "tr"] || place.name?.tr || place.slug || "";

                    return (
                      <div key={place.slug || placeIndex} className="relative pl-5 border-l-2 border-gray-100 group/place">
        ...
        {visibleCount < countryCities.length && (
