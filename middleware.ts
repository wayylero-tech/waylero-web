...
// 🛡️ SENİN DEVASA LİSTEN (HİÇBİRİNİ SİLMEDİM KANKA)
...
export function middleware(request: NextRequest) {
  ...
  // 🔥 1. ADIM: DİL ÖN EKİ KONTROLÜ
  // Eğer link zaten /en/ veya /de/ ile başlıyorsa, dokunmuyoruz. 
  // Çünkü onları next.config.ts içindeki "rewrites" sessizce halledecek.
  ...
  // URL temizleme (boşluk sil, küçük harf yap, karakter düzelt)
  const sanitize = (str: string) => str.toLowerCase()
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    .replace(/ /g, '')
    .replace(/ı/g, 'i').replace(/ğ/g, 'g').replace(/ü/g, 'u').replace(/ş/g, 's').replace(/ö/g, 'o').replace(/ç/g, 'c');

  // 🔥 2. ADIM: MEVCUT YÖNLENDİRME MANTIĞIN (1300 İNDEKSİ KORUYAN KISIM)
  
  // --- DURUM 1: Mekan Sayfaları ---
  ...
  // --- DURUM 2: Şehir Sayfaları (3 Parçalı) ---
  ...
  // --- DURUM 3: Kısa Şehir Linkleri (2 Parçalı) ---
...
export const config = {
  // Statik dosyaları (görsel, favicon vb.) middleware'den muaf tutuyoruz.
