...
const files = [
  "app/data/turkey.json",
  "app/data/europa.json",
  "app/data/asia.json",
];

// 🌍 ŞEHİR → ÜLKE MAP (sende zaten var ama buraya koydum)
...
for (const filePath of files) {
  ...
  Object.entries(json).forEach(([city, places]) => {
    if (!Array.isArray(places)) return;

    // 🔥 COUNTRY ARTIK DOSYADAN DEĞİL CITY’DEN GELİYOR
    const country = cityToCountryMap[city] || "unknown";
    ...
    places.forEach((place) => {
      globalPlaces.push({
        ...
        name_tr: place.name?.tr,
        name_en: place.name?.en,
...
fs.writeFileSync(
  "app/data/globalPlaces.json",
  JSON.stringify(globalPlaces, null, 2)
);

console.log("DONE ✅ Total places:", globalPlaces.length);